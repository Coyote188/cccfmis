...
public class ApplictionPDPZWindow
		extends Window
{
	...
	private void init(){
		Div div = (Div) this.getFellow("divCertification");
		Map map = new HashMap();
		map.put("applyId", application.getId());
		Window objWin = (Window) Executions.createComponents("/SysForm/technicalevaluation/jspd_fileprinter.zul", div, map);
		objWin.doEmbedded();
	}
			...
			throws SuspendNotAllowedException, InterruptedException
	{
		...
		wn.setTitle("[" + application.getProduction().getProductName() + "]评定报靠");
		Iframe ss = (Iframe) wn.getFellow("ifrme");
		ss.setSrc("/cccfpage/pdffileModel/report/" + rowId + "/pdbg.pdf");
		wn.doModal();
	}
	public void onSeezs()
			throws SuspendNotAllowedException, InterruptedException
	{
		...
		Iframe ss = (Iframe) wn.getFellow("ifrme");
		ss.setSrc("/cccfpage/pdffileModel/report/" + rowId + "/zs.pdf");
		wn.doModal();
	}
