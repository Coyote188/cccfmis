		...
		extends GenericForwardComposer
{
			...
			throws Exception
	{
		...
		this.msgList = MessageInfoServiceUtil.findMessage(2, user);
		...
		EventQueues.lookup(user.getId() + "msgEventQueue", EventQueues.APPLICATION, true).subscribe(new EventListener()
		{
					...
					throws Exception
			{
				...
				if (msg.getType() == 1)
				{
					...
					Window objWindow = (Window) Executions.createComponents("mytask_list_enterprise.zul", null, null);
		...
		// listener of notice
		EventQueues.lookup(user.getId() + "noticeEventQueue", EventQueues.APPLICATION, true).subscribe(new EventListener()
		{
			@Override
			public void onEvent(Event event)
					throws Exception
	...
	public void onClick$actiTob(ForwardEvent event)
			throws SuspendNotAllowedException, InterruptedException
	{
		...
		} else
		{
			...
			openEmbedWindow(tabname, "/SysForm/commontreaty.zul", param);
			// Window obj = (Window) Executions.createComponents("/SysForm/commontreaty.zul", null, param);
			// try {
			// obj.doModal();
			// } catch (Exception e) {
	...
	{
		epCTbx.getSelectedTab().close();
	}
	public void onViewMsg(ForwardEvent event)
	{
		...
		subject.setValue(msg.getSubject());
		content.setValue(msg.getContent());
		msgpop.open(event.getOrigin().getTarget(), "after_start");
		MessageInfoServiceUtil.readMsg(msg);
		Toolbarbutton item = (Toolbarbutton) event.getOrigin().getTarget();
		// item.setStyle("color:gray;text-Decoration:underline");
		Listcell cell = (Listcell) item.getParent().getParent().getFirstChild();
		cell.setStyle("color:gray");
		// ListModelList model = new ListModelList(this.msgList);
		// if(!this.msgList.isEmpty())
		// lbxMsgList.setModel(model);
	}
	/**
	 * 已激活产品
	 * 
	 ...
	 * @throws InterruptedException
	 */
	public void onClick$actdTob(ForwardEvent event)
			throws SuspendNotAllowedException, InterruptedException
	{
		if (!getActivedStatus())
		{
	...
	public void onClick$madeTob(ForwardEvent event)
			throws SuspendNotAllowedException, InterruptedException
	{
		if (!getActivedStatus())
		{
	...
	{
		if (findTab(title) == -1)
			createTabWithTabpanel(title, url, param);
		else
		{
			...
			Window objWindow = (Window) Executions.createComponents(url, null, param);
	...
	public void createTabWithTabpanel(String tabname, String url, Map param)
	{
		Tab t = new Tab();
		t.setLabel(tabname);
		t.setClosable(true);
	...
	public void onClick$btnMailBox()
	{
		...
		try
		{
			...
			Window objWindow = (Window) Executions.createComponents("/SysForm/EnterpriseNotice/notice-form.zul", null, null);
			objWindow.doModal();
	...
	public void onClick$infoTob(ForwardEvent event)
			throws SuspendNotAllowedException, InterruptedException
	{
		if (!(enterprise.getStatus() == ActivateStatus.退回.ordinal()))
			if (!getActivedStatus())
			{
				return;
			}
	...
	public void onClick$prodTob(ForwardEvent event)
			throws SuspendNotAllowedException, InterruptedException
	{
		if (!getActivedStatus())
		{
	...
	public boolean getActivedStatus()
	{
		...
		if (enterprise.getStatus() == ActivateStatus.未激活.ordinal() || ActivateStatus.退回.ordinal() == enterprise.getStatus())
		{
			try
			{
				Messagebox.show("企业账号还未激活,不能使用该项功能!", "操作提示", Messagebox.OK, Messagebox.ERROR);
				isActied = false;
			} catch (InterruptedException e)
			{
	...
	//
	public void onClick$execTob(ForwardEvent event)
			throws SuspendNotAllowedException, InterruptedException
	{
		if (!getActivedStatus())
		{
		...
		String tabname = ((Toolbarbutton) event.getOrigin().getTarget()).getLabel();
		openEmbedWindow(tabname, "application-apply.zul", null);
	}
