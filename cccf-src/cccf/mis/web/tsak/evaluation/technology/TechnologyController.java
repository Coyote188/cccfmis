package cccf.mis.web.tsak.evaluation.technology;
		...
		extends GenericForwardComposer
{
	...
	private Textbox  opinionContent;
	private Combobox cb_approveResult;
	private Textbox  approveMemo;
	
	TechnicalEvaluationResultUpService svc = BeanAdapter.getBean("TechnicalEvaluationResultUpService", TechnicalEvaluationResultUpService.class);
			...
			throws InterruptedException
	{
		if(Messagebox.show("是否确认提交表单信息？","操作提示",Messagebox.OK|Messagebox.NO,Messagebox.QUESTION)==Messagebox.OK)
		{
			...
			opinion.setTechnicalEvaluationReport(new TechnicalEvaluationReport());
			opinion.getTechnicalEvaluationReport().setReportNo((String)selectItem.get("reportNo"));
			opinion.setEvaluationPersonId( UserInfoServiceUtil.getCurrentLoginUser().getId());
			opinion.setApproveResult((String) cb_approveResult.getSelectedItem().getValue());
			opinion.setOpinionContent(opinionContent.getText());
			opinion.setMemo(approveMemo.getText());
		 
			svc.doEvaluationResultUp(opinion, "提交", opinion.getMemo());
