...
public class EvaluationTaskUtils
{
	...
	{
		StringBuffer hql = new StringBuffer("select DISTINCT new map(o.contractNo as contractno")
				// 合同号
				...
				// 报告编号
				.append(",case when pl.end is null then '处理' else '查看' end as butlabel ").append(")").append(" from Contract o ,ApplicationInfo a ,ProcessLog pl ,TechnicalEvaluationReport r").append(" where o.applyno = a.sioid ")
				.append(" and pl.boId = a.id ").append(" and o.applyno  = r.applyno ").append(" and pl.taskInstanceName='").append(task_name).append("' ").append(getWhere(task_name, args));
	...
	private static StringBuffer getWhere(String task_name, Map<String, String> args)
	{
		...
		if (args.get("status") != null && !args.get("status").isEmpty())
		{
			if ("0".equals(args.get("status")))
			{
				...
				{
					StringBuffer ids = CommonFunction.getBacklogs(args.get("usid"), task_name);
					hql.append(" and ( pl.taskInstanceId in ").append(ids).append(" or pl.actorId='").append(args.get("usid")).append("'").append(")");
				}
			} else
			{
	...
	{
		StringBuffer hql = new StringBuffer("select count(DISTINCT r.reportNo )").append(" from Contract o ,ApplicationInfo a ,ProcessLog pl,TechnicalEvaluationReport r").append(" where o.applyno = a.sioid ").append(" and pl.boId = a.id ")
				.append(" and o.applyno  = r.applyno ").append(" and pl.taskInstanceName='").append(task_name).append("' ").append(getWhere(task_name, args));
		ContractService contractService = BeanAdapter.getBean("ContractService", ContractService.class);
